"""
Generating target function, featurizer, and train/validation/test splits for
the Lasso homework.
...
def get_target_and_featurizer(num_basis_fns=100,
                              num_nonzero=10,
                              coefs_true=None):
...
def load_problem(file_name):
    ...
    f_myfile.close()
    return reconstitute_problem(data["coefs_true"], data["x_train"],
...
def main():
    ...
    ax.plot(x,
            target_fn(x),
            'r',
            label='Target function (i.e. Bayes prediction function)')
